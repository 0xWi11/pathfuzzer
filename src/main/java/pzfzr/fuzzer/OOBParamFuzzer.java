...
/**
 * OOBParamFuzzer - 专门用于OOB（Out-of-Band）参数模糊测试
 * 使用HEADER_PAYLOAD_INFOS进行测试，与其他类共享生命周期
 */
public class OOBParamFuzzer {
            ...
            nettyManager.sendRequest(modifiedRequest, new NettyManager.ResponseCallback() {
                ...
                public void onResponse(HttpResponse response, long responseTimeMs) {
                    try {
                        ...
                        logging.logToError("[OOBParamFuzzer] Error processing response for ID " + tempID + ": " + e.getMessage());
                    }
                }

                @Override
                public void onError(Throwable error) {
                    logging.logToError("[OOBParamFuzzer] Request failed for ID " + tempID + ": " + error.getMessage());
                    // 可以在这里更新表格状态为错误状态
                }
            });

        } catch (Exception e) {
            logging.logToError("[OOBParamFuzzer] sendTestRequestAsync error: " + e.getMessage());
        }
    }
    ...
    public void setShuttingDown(boolean shuttingDown) {
        ...
        if (shuttingDown) {
            logging.logToOutput("[OOBParamFuzzer] Starting shutdown...");
    ...
    private JsonNode modifyJsonNode(JsonNode rootNode, String path, String newValue, String payloadAlias) throws Exception {
        ...
        // OOB测试通常使用字符串payload
        JsonNode valueToSet = new TextNode(newValue);
